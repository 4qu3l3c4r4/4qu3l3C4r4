...
public class CommonActions {
	...
	/**
	* Método responsável para a abertura de uma URL.
	* Utiliza a url para abrir uma página web.
	*/
	...
	* Utiliza o By informado para localizar o componente.
	* Capaz de ignorar erros do tipo NoSuchElementException e StaleElementReferenceException
	...
	*/
	...
	WebElement element = null;
	Wait<WebDriver> wait = new FluentWait<WebDriver>(driver).withTimeout(5, TimeUnit.SECONDS)/*Esperando 25 segundos para que o elemento esteja presente na página*/
	...
	public boolean selectDropdown(WebDriver driver, By xpath, String opcaoSelecionada) {
		...
		for (int item = 1; item <= quantidadeDeItens; item++) {
			...
			if (opcaoSelecionada.toUpperCase().trim().equals(descItem.toUpperCase())) {
				...
				click(driver, By.xpath("//p-dropdown[@id='drpDwnProcessTypes']/div/div[3]/div/ul/li[" + item + "]/span"));
				return true;
			}
		}
		return false;
